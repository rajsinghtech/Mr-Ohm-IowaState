...
from .camera_test import WishboneWriteModule
...
class OrbSoC(SoCCore):
    ...
    def add_auto_reset(self):
        ...
        btn = self.platform.request('rst_n')
    ...
    def add_boot_software(self):
        mem_map = {
            "app": 0x30000000,
        }

        self.mem_map.update(mem_map)
        # self.add_memory_region("camera_sub", getattr(self.mem_map, "camera_sub", None), 0x10000000)
        self.add_rom("app", self.mem_map["app"], 128 * 1024)


    def add_camera_sub(self):
        mem_map = {
            "camera_sub": 0x20000000
        }
        self.mem_map.update(mem_map)
        # self.add_memory_region("camera_sub", getattr(self.mem_map, "camera_sub", None), 0x10000000)
        

        self.add_memory_region("camera_sub", self.mem_map["camera_sub"], 0x1000)

        # Instantiate a Wishbone bus interface
        mod = WishboneWriteModule(self.mem_map["camera_sub"], 0x1000,self.bus.address_width)


        # add the wishbone slave mapping to the mem_map entry made above
        self.add_wb_slave(self.mem_map["camera_sub"], mod.bus)

        # Instantiate the WishboneWriteModule
        self.submodules.cam_sub = mod 
    ...
    def add_adc(self):
        ...
        self.clock_domains.adc = ClockDomain()

        self.platform.add_source('verilog/adc_top.v')
        self.platform.add_source('verilog/sigmadelta_adc.v')
        self.platform.add_source('verilog/box_ave.v')
        ...
        self.add_memory_region("adc", self.mem_map["adc"], 0x80)

        self.cmp = self.platform.request("adc_cmp", 0).p


        # Instantiate a Wishbone bus interface
        mod = ADCModule(self.mem_map["adc"], self.cmp, 0x80, self.bus.address_width)


        # add the wishbone slave mapping to the mem_map entry made above
        self.add_wb_slave(self.mem_map["adc"], mod.bus)

        # Instantiate the WishboneWriteModule
        self.submodules.adc_mod = mod 
