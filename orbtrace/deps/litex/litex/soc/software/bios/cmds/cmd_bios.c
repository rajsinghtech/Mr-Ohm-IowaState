 ...
 *
 */
#ifdef CSR_LED_CTRL_BASE
static void leds_ctrl_handler(int nb_params, char **params)
{
	char *c;
	unsigned int value;
	...
	printf("Settings Leds to 0x%x", value);
	led_ctrl_set_write(value);
}

define_command(leds_ctrl, leds_ctrl_handler, "Set Value to LED Controller", SYSTEM_CMDS);
#endif
...
// static void uart_shell_handler(int nb_params, char **params)
// {
// 	int exit_index = 0;
// 	while(1) {
...
// 		if(c == EXIT_STRING[exit_index]) {
// 			exit_index++;
// 			if(exit_index == strlen(EXIT_STRING)) {
// 				break;
// 			}
// 		} else {
// 			exit_index = 0;
...
// define_command(uart_shell, uart_shell_handler, "Start a uart shell, to exit type 'exit'", SYSTEM_CMDS);
// #endif
